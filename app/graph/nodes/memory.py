...
from app.graph.constants import MORE_OPTIONS_ID, MORE_OPTIONS_TITLE
...
def memory_node(state: AgentState):
    ...
    if last_msg.strip() in (MORE_OPTIONS_ID, MORE_OPTIONS_TITLE):
        ...
        return {
            "district": existing_profile.get("district"),
            "block": existing_profile.get("block"),
            "village": existing_profile.get("village"),
            "language": existing_profile.get("language") or "English",
            "user_name": existing_profile.get("full_name", "Friend"),
    ...
    try:
        ...
        return {
            ...
            # A genuine (non-"More Options") message means we're either newly
            # arriving at a location level or asking something unrelated --
            # either way, reset pagination back to page 0 for whatever list
            # gets shown next.
            "location_offset": 0,
        }

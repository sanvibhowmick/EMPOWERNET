...
# FIX (weak point #6/#7): the dashboard used to open its own brand-new raw
# psycopg2 connection on every single `fetch()` call (this file makes dozens
# of them per page render) and tear it down immediately after -- no pooling
# at all, and yet another DB-access pattern distinct from the rest of the
# app. `fetch()` now checks a connection out of the same shared, pooled
# SQLAlchemy engine every other module uses (via `engine.raw_connection()`),
# and always returns it to the pool. It's still a raw psycopg2-style DBAPI
# connection under the hood (so every existing `%s`-placeholder query below
# keeps working unchanged) -- only the connection *lifecycle* changed, from
# "open a fresh TCP+auth handshake per call" to "borrow from the pool."
from app.core.db import engine

# FIX (weak point #11): district names and centroids used to be hardcoded
# here as `DC`, using a *different* naming convention than data/jobs.py and
# data/train.py's mock-data generators (e.g. "PURBA MEDINIPUR" here vs.
# "MEDINIPUR EAST" there) -- silently orphaning every mock job/training row
# from this dashboard's choropleth map. All three now import the same
# canonical list from app/core/districts.py.
from app.core.districts import DISTRICT_CENTROIDS as DC, ALL_DISTRICTS
...
# NOTE (weak point #15 -- NOT fully fixed, documented limitation): these are
# still hand-rolled rectangular bounding boxes, not real district polygons.
# Fixing this properly requires sourcing an actual West Bengal district
# shapefile/GeoJSON and is out of scope for this pass -- flagging it
# explicitly here rather than leaving it silently unfixed. `DC` (centroids)
# and `ALL_DISTRICTS` now come from app/core/districts.py (imported at the
# top of this file) instead of being redefined here.
...
def get_district_kpis(d):
    ...
    # FIX (weak point #12, dashboard side): this used to filter safety
    # reports ONLY by a lat/lon bounding box. app/tools/reporting.py's
    # submit_safety_report never wrote lat/lon at all, so this query always
    # returned zero for every report actually filed through the real bot --
    # the count was structurally guaranteed to be wrong, not just
    # occasionally stale. Reports now carry district text (always present)
    # plus lat/lon when resolvable (see app/tools/spatial.py's
    # get_village_coordinates); this counts a report if EITHER matches, so
    # historical district-only reports are counted too, not just newly
    # geocoded ones.
    return (int(fetch_one("SELECT COUNT(*) n FROM self_help_groups WHERE UPPER(district)=%s",(d,)).get("n",0)),
            ...
            int(fetch_one(
                "SELECT COUNT(*) n FROM safety_reports WHERE UPPER(district)=%s "
                "OR (lat BETWEEN %s AND %s AND lon BETWEEN %s AND %s)",
                (d, clat-.7,clat+.7,clon-.7,clon+.7)
            ).get("n",0)))
...
def get_safety_geo(d=None):
    # Only reports with resolvable coordinates can actually be plotted as
    # points on the map, so this stays lat/lon-scoped for the map layer --
    # unlike get_district_kpis's *count*, a district-text match with no
    # coordinates has nothing to plot. See get_district_kpis's comment for
    # why lat/lon alone used to always be empty for bot-submitted reports;
    # that's fixed at the source now (app/tools/reporting.py), so this will
    # start returning real points for newly filed reports.
    if d:
        clat,clon=DC.get(d,(23,87.8))
        return fetch("SELECT COALESCE(category,'—') cat,description,lat,lon FROM safety_reports WHERE lat IS NOT NULL AND lon IS NOT NULL AND lat BETWEEN %s AND %s AND lon BETWEEN %s AND %s LIMIT 400",(clat-.7,clat+.7,clon-.7,clon+.7))
    return fetch("SELECT COALESCE(category,'—') cat,description,lat,lon FROM safety_reports WHERE lat IS NOT NULL AND lon IS NOT NULL LIMIT 600")
...
def get_safety_cats(d=None):
    if d:
        clat,clon=DC.get(d,(23,87.8))
        return fetch(
            "SELECT COALESCE(category,'Other') cat,COUNT(*) cnt FROM safety_reports "
            "WHERE UPPER(district)=%s OR (lat BETWEEN %s AND %s AND lon BETWEEN %s AND %s) "

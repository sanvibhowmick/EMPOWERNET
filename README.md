...
└── requirements.txt
🚩 The Safety Workflow
User Reports Hazard: A worker sends a message or voice note in Bengali via WhatsApp.

Transcription/Translation: whisper.py and the Reporting Node convert the message to an English summary.

Database Impact: reporting.py logs the entry and automatically lowers the safety score of workplaces within the proximity.

Community Protection: jobs.py immediately begins filtering out any workplace that drops below the safety threshold in future searches.